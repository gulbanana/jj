...
// TODO: WorkspaceCommandEnvironment is vestigial. It exists because templating is part of the CLI,
// but template parsing & usage code is coupled with the other workspace state that resides in lib.
...
",
    ...
    pub fn check_rewritable_expr(
        ...
        let Some(commit_id) = self.env.find_immutable_commit(
            repo,
            to_rewrite_expr,
    ...
    fn finish_transaction(
        ...
        for (name, wc_commit_id) in &tx.repo().view().wc_commit_ids().clone() {
            ...
            let is_immutable = match self.env.find_immutable_commit(
                tx.repo(),
                &wc_expr,
                self.env.command.global_args().ignore_immutable,
            ) {
